            ...
            {//Not in offline mode
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        //if for example the tag type did not exist on the online database, so the Create() function
                        //needed to be able to account for that.
                        AppLog.Info("CREATE TAGTYPE - Checking that tag type doesn't already exist on online database");

                        bool OnlineTagTypeExists;

                        SqlCommand CheckOnlineDb = new SqlCommand("SELECT * FROM t_TagTypes WHERE Id = @Id", conn);
                        CheckOnlineDb.Parameters.Add(new SqlParameter("Id", Id));
                        using (SqlDataReader reader = CheckOnlineDb.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                OnlineTagTypeExists = true;
                                AppLog.Info("CREATE TAGTYPE - TagType already exists in online database!");
                            }
                            else
                            {
                                OnlineTagTypeExists = false;
                                AppLog.Info("CREATE TAGTYPE - TagType does not exist in online database. Creating tag type on online database");
                            }
                        }

                        if (!OnlineTagTypeExists)
                        {
            ...
            if (!Data.OfflineMode)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        SqlCommand UpdateTagType = new SqlCommand("UPDATE t_TagTypes SET Value = @Value WHERE " +
                            "Id = @Id;", conn);
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        SqlCommand DownloadTagType = new SqlCommand("SELECT * FROM t_TagTypes WHERE Id = @Id;", conn);
