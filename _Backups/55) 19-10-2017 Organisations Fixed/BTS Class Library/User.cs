            ...
            if (Data.OfflineMode)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        AppLog.Info(String.Format("GET USER - {0} organisations were retrieved for user from local database " +
            ...
            else //If offline mode is off, latest copy of user will be downloaded from the online database
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        AppLog.Info(String.Format("GET USER - {0} organisations were retrieved for user from online database " +
                            "successfully", DownloadedOrganisations));
                    }
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        using (SqlDataReader reader = LogInUserOffline.ExecuteReader())
                        {
                            ...
                            {
                                //If reader.Read() returns false, no data was returned
                                _ErrMsg = "Error while logging user in from local database. No data was returned";
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        using (SqlDataReader reader = LogInUserOnline.ExecuteReader())
                        {
                            ...
                            {
                                //If reader.Read() returns false, no data was returned.
                                _ErrMsg = "Error while logging user in from online database. No data was returned";
                                AppLog.Error("LOGIN USER - " + _ErrMsg);
                                return false;
                            }
                        }
