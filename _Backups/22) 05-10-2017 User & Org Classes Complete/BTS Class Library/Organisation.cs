...
namespace BTS_Class_Library
{
    public partial class Organisation
    {
        ...
        public bool Create()
        {
            ...
            {
                //If this fails, we need to undo the create command on the online database
                //by doing a delete
                _ErrMsg = "Error while creating organisation on local database. Organisation will now be deleted";
                AppLog.Info("Attempting to resolve error by deleting organisation...");
                if (Delete())
                {
                    AppLog.Info("Organisation deleted. Error resolved successfully!");
                }
                else
                {
                    AppLog.Info("Organisation not deleted. FATAL ERROR!!!");
                    throw new Exception("Fatal error while creating organisation. Could not revert changes.");
                }
                AppLog.Error(_ErrMsg + ": " + e);
                return false;
            }
        ...
        public bool Update()
        {
            ...
            //If org was backed up successfully, try update online database
                    ...
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        using (SqlDataReader reader = GetOrganisation.ExecuteReader())
                        {
                            ...
                            else
                            {
                                ...
                                _ErrMsg = "Error while downloading organisation from local database. No data was returned";
                                AppLog.Error(_ErrMsg);
                                return false;
                            }
                        }
