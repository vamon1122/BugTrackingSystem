...
namespace BTS_Class_Library
{
    public class User
    {
        ...
        public bool Create()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                {
                    ...
                    SqlCommand CreateUser = new SqlCommand("INSERT INTO t_Users VALUES(@Id, @FName, @SName, " +
        ...
        public bool Update()
        {
            ...
            {
                /* - Realised that it doesn't really matter if it fails on the local db. A
                 * note in the log would suffice
                 * 
        ...
        private bool Validate()
        {
            ...
            try
            {
                if(FName == null) { throw new Exception("User has not been given a forename"); }
                if(FName.Length > 50) { throw new Exception("Forename exceeds 50 characters"); }
                if(SName == null) { throw new Exception("User has not been given a surname"); }
                if(SName.Length > 50) { throw new Exception("Surname exceeds 50 characters"); }
                if(Username == null) { throw new Exception("User has not been given a username"); }
                if (Username.Length > 50) { throw new Exception("Username exceeds 50 characters"); }
                if (JobTitle == null) { throw new Exception("User has not been given a job title"); }
                if (JobTitle.Length > 50) { throw new Exception("Job title exceeds 50 characters"); }
                if (EMail == null) { throw new Exception("User has not been given a email address"); }
                if (EMail.Length > 50) { throw new Exception("Email address exceeds 50 characters"); }
                if (Password == null) { throw new Exception("User has not been given a password"); }
                if (Password.Length > 50) { throw new Exception("Password exceeds 50 characters"); }
            }
            catch (Exception e)
            {
                _ErrMsg = e.ToString();
