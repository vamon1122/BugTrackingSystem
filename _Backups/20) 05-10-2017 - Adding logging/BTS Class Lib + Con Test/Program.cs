...
namespace BTS_Class_Lib___Con_Test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Testing Offline Mode");
            Console.ResetColor();
            ...
            Console.WriteLine("OFFLINE MODE WAS TURNED ON");
            Console.ResetColor();
            Console.WriteLine();
            ...
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("OFFLINE MODE WAS TURNED OFF");
            Console.ResetColor();
            Console.WriteLine();

            if (MyUser.Delete()) { Console.WriteLine("User deleted successfully!"); } else { Data.UserFriendlyError(MyUser.ErrMsg); }

            User MyDownloadedUser3 = new User(MyUserGuid);

            Console.WriteLine();
            Console.ForegroundColor = ConsoleColor.Green;
            Console.Write("Attempting to download a record which doesn't exist. An error SHOULD be produced");
            Console.ResetColor();
            Console.WriteLine(" (Press enter to dismiss)");
            Console.ResetColor();
            Console.ReadLine();

            if (MyDownloadedUser3.Get()) { Console.WriteLine("User downloaded successfully!"); } else { Data.UserFriendlyError(MyDownloadedUser3.ErrMsg); }
