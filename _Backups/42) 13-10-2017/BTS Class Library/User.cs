...
namespace BTS_Class_Library
{
    public class User
    {
        ...
        public bool Create()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                {
                    ...
                    SqlCommand CreateUser = new SqlCommand("INSERT INTO t_Users (Id, FName, SName, DOB, Gender, " +
                        "Username, JobTitle, EMail, Password, DateTimeCreated) VALUES(@Id, @FName, @SName, @DOB, " +
                        "@Gender, @Username, @JobTitle, @EMail, @Password, @DateTimeCreated);", conn);
                    ...
                    CreateUser.Parameters.Add(new SqlParameter("DOB", _DOB));
                    CreateUser.Parameters.Add(new SqlParameter("Gender", _Gender));
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                {
                    ...
                    SqlCommand CreateUser = new SqlCommand("INSERT INTO Users VALUES(@Id, @FName, @SName, @DOB, " +
                        "@Gender, @Username, @JobTitle, @EMail, @Password, @DateTimeCreated);", conn);
                    CreateUser.Parameters.Add(new SqlParameter("Id", _Id));
                    CreateUser.Parameters.Add(new SqlParameter("FName", _FName));
                    CreateUser.Parameters.Add(new SqlParameter("SName", _SName));
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                {
                    ...
                    SqlCommand UpdateUser = new SqlCommand("UPDATE t_Users SET FName = @FName, SName = @SName," +
                        "DOB = @DOB, Gender = @Gender, Username = @Username, JobTitle = @JobTitle, EMail = @EMail, " +
                    ...
                    UpdateUser.Parameters.Add(new SqlParameter("DOB", _DOB));
                    UpdateUser.Parameters.Add(new SqlParameter("Gender", _Gender));
                    ...
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        using (SqlDataReader reader = GetUser.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                _Password = reader[9].ToString().Trim();
                                _DateTimeCreated = Convert.ToDateTime(reader[13]);
                            }
                            else
                            {
        ...
        private bool Validate()
        {
            ...
            if (FName == null) { _ErrMsg = "User has not been given a forename"; return false; }
            if (FName.Length > 50) { _ErrMsg = "Forename exceeds 50 characters"; return false; }
            if (SName == null) { _ErrMsg = "User has not been given a surname"; return false; }
            if (SName.Length > 50) { _ErrMsg = "Surname exceeds 50 characters"; return false; }
            if (DOB.ToString() == new DateTime().ToString()) { _ErrMsg = "User has not been given a date of birth";
                return false; }
            if (Gender.ToString() != "M" && Gender.ToString() != "F" && Gender.ToString() != "O")
                { _ErrMsg = "No gender was selected"; return false; }
            if (Username == null) { _ErrMsg = "User has not been given a username"; return false; }
            if (Username.Length > 50) { _ErrMsg = "Username exceeds 50 characters"; return false; }
            if (JobTitle == null) { _ErrMsg = "User has not been given a job title"; return false; }
            if (JobTitle.Length > 50) { _ErrMsg = "Job title exceeds 50 characters"; return false; }
            if (EMail == null) { _ErrMsg = "User has not been given a email address"; return false; }
            if (EMail.Length > 50) { _ErrMsg = "Email address exceeds 50 characters"; return false; }
            if (Password == null) { _ErrMsg = "User has not been given a password"; return false; }
            if (Password.Length > 50) { _ErrMsg = "Password exceeds 50 characters"; return false; }
            ...
            if (Data.OfflineMode)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        SqlCommand GetUser = new SqlCommand("SELECT * FROM Users WHERE Username = @Username " +
            ...
            else //If offline mode is off, latest copy of user will be downloaded from the online database
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        SqlCommand DownloadUser = new SqlCommand("SELECT * FROM t_Users WHERE Username = @Username " +
                            "AND Password = @Password;", conn);
