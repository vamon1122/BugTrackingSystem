    ...
    public class Note
    {
            ...
            if (!Data.OfflineMode)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        SqlCommand CheckOnlineDb = new SqlCommand("SELECT * FROM t_BTS_Notes WHERE Id = @Id", conn);
            ...
            if (!Data.OfflineMode)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        SqlCommand UpdateNote = new SqlCommand("UPDATE t_BTS_Notes SET EditedById = @EditedById, " +
        ...
        public bool Get()
        {
            ...
            else
            {
                ...
                //Finally, check if note exists in the local database. If not, ADD THEM!!! If so, UPDATE THEM!!!
