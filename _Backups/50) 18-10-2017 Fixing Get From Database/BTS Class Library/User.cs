                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        ...
                        if(Username != null && Username != "")
                        {
                            ...
                            GetUser = new SqlCommand("SELECT Id FROM Users WHERE Username = @Username " +
                        ...
                        else if(EMail != null && EMail != "")
                        {
                            ...
                            GetUser = new SqlCommand("SELECT Id FROM Users WHERE EMail = @EMail " +
                            "AND Password = @Password;", conn);
                            GetUser.Parameters.Add(new SqlParameter("EMail", _EMail));
                        }
                        else
                        {
                    ...
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        using (SqlDataReader reader = DownloadUser.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                if (Get())
                                {
                                    AppLog.Info("LOGIN USER - Got user successfully");
                                }
                                else
                                {
                                    AppLog.Error("LOGIN USER - Failed to get user: " + ErrMsg);
                                    return false;
                                }
                            }
                            else
