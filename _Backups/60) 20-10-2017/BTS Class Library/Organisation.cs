    ...
    public partial class Organisation
    {
        ...
        public List<OrgMember> Members { get {
                ...
                if (Data.OfflineMode)
                {
                    ...
                    AppLog.Info("MEMBERS (Getter) - Attempting to retrieve organisation's members from local database...");

                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        AppLog.Info("MEMBERS (Getter) - Attempting to open connection to local database...");
                        conn.Open();
                        AppLog.Info("MEMBERS (Getter) - Connection to local database opened successfully");
                        ...
                        AppLog.Info("MEMBERS (Getter) - OrgMembers downloaded from local database successfully");
                    }
                }
                else
                {
                    ...
                    {
                        AppLog.Info("PRODUCTS (Getter) - Attempting to open connection to online database...");
                        conn.Open();
                        AppLog.Info("PRODUCTS (Getter) - Connection to online database opened successfully");
                        AppLog.Info("PRODUCTS (Getter) - Attempting to retrieve organisation's members from online database...");
                        SqlCommand GetOrgMembers = new SqlCommand("SELECT UserId FROM t_OrgMembers WHERE OrgId = @OrgId", conn);
                        GetOrgMembers.Parameters.Add(new SqlParameter("OrgId", Id));
                    ...
                    AppLog.Info("MEMBERS (Getter) - OrgMembers downloaded from online database successfully");
                }
                return OrgMembers;
            }
        }
        ...
        public List<Product> Products { get {
                ...
                AppLog.Info("PRODUCTS (Getter) - Attempting to retrieve organisation's products from local database...");
                ...
                if (Data.OfflineMode)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(Data.LocalConnStr))
                    {
                        AppLog.Info("PRODUCTS (Getter) - Attempting to open connection to local database...");
                        conn.Open();
                        AppLog.Info("PRODUCTS (Getter) - Connection to local database opened successfully");
                    ...
                    AppLog.Info("PRODUCTS (Getter) - Products downloaded from local database successfully");
                }
                else
                {
                    using (SqlConnection conn = new SqlConnection(Data.OnlineConnStr))
                    {
                        ...
                        conn.Open();
                        AppLog.Info("PRODUCTS (Getter) - Connection to online database opened successfully");
                        AppLog.Info(String.Format("PRODUCTS (Getter) - Organisation {0} downloaded from online database successfully", _Name));

                        AppLog.Info("PRODUCTS (Getter) - Attempting to retrieve organisation's products from online database...");
                        SqlCommand GetProducts = new SqlCommand("SELECT Id FROM t_Products WHERE OrgId = @OrgId", conn);
                        GetProducts.Parameters.Add(new SqlParameter("OrgId", Id));
                        ...
                        AppLog.Info("PRODUCTS (Getter) - Products downloaded from online database successfully");
                    }
                }
                return MyProducts;
            }
