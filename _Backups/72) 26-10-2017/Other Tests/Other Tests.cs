...
namespace Other_Tests
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Bug MyBug = new Bug(new Guid("b3e7eb2c-52bd-4acd-ab0f-e05258e09737")); //Multi Photo bug
            if (!MyBug.Get())
            {
                Console.WriteLine(MyBug.ErrMsg);
            }

            Organisation MyOrg = new Organisation(new Guid("3fab6188-b010-45ed-bbec-ee6976d2fec9"));
            if (!MyOrg.Get())
            {
                Console.WriteLine(MyOrg.ErrMsg);
            }

            TagType MyTagType = MyOrg.NewTagType();
            MyTagType.Value = "New Tag 5";
            if (!MyTagType.Create())
            {
                Console.WriteLine(MyTagType.ErrMsg);
            }

            TagType MyDownloadedTagType = new TagType(MyTagType.Id);
            MyDownloadedTagType.Get();
            Console.WriteLine("Downloaded TagType value: " + MyDownloadedTagType.Value);
            //Console.ReadLine();

            Tag MyTag = MyBug.NewTag(MyTagType);
            if (!MyTag.Create())
            {
                Console.WriteLine(MyTag.ErrMsg);
            }
